...
vo = collections.deque([0]*(muestraD - MinDatosFiltro), maxlen=(muestraD - MinDatosFiltro))
data_rest = collections.deque([0]*(muestraD - MinDatosFiltro), maxlen=(muestraD - MinDatosFiltro))
...
try:
    ...
    def Iniciar():
        ...
        estaEjecutandose = True
        estaRecibiendo = True
        animacion = anim.FuncAnimation(fig, plotData, fargs=(muestraD, MinDatosFiltro, contador, lines1, lines2, lines3), interval=1000, blit=False)
        plt.legend()
        plt.show()
        global thread
    ...
    def plotData(self,muestraD, MinDatosFiltro, contador1, lines1, lines2, lines3):
    ...
    lines1 = ax.plot([], [], 'b', label = "Señal entrada")[0]
    lines2 = ax.plot([], [], 'r', label = "Señal fondo")[0]
    lines3 = ax.plot([], [], '#5CFE05', label = "Señal local")[0]
    ...
    def Guardar():
        ...
        time.sleep(0.2)
        archivo.write("Tiempo" + "," + "Concentracion" + "," + "SenalFondo" + "\n")
        while (estaGuardando):
            time.sleep(1)
            archivo.write(str(round(time.time())) + "," + str(datos) + "," + str(round(vo[-1], 2)) + "\n")
